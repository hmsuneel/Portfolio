...
**Two files now, not one:** `index.html` and `avatar.jpg` (your photo,
shown in a circular frame in the hero). Both need to be uploaded together
and sit in the same folder — the page references the photo as `avatar.jpg`,
so don't rename it unless you also update that reference in `index.html`
(search for `src="avatar.jpg"`).
